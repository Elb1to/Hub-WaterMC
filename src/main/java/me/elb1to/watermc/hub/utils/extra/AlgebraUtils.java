...
public class AlgebraUtils {
	...
	public static Vector getAverageBump(Location source, List<Location> locs) {
		...
		for (Location loc : locs) {
			vec.add(AlgebraUtils.getTrajectory(loc, source));
			amount++;
		}
	...
	public static Entity findClosest(Entity mid, Collection<Entity> locs) {
		...
		for (Entity loc : locs) {
			double dist = MathUtils.offsetSquared(mid, loc);
	...
	public static boolean isTargetInPlayerPyramid(Player player, Player target, double angleLimit) {
		return isInPyramid(player.getLocation().getDirection(), AlgebraUtils.getTrajectory(player.getEyeLocation(), target.getEyeLocation()), angleLimit) ||
				isInPyramid(player.getLocation().getDirection(), AlgebraUtils.getTrajectory(player.getEyeLocation(), target.getLocation()), angleLimit);
	}
	...
	public static Location getLocationAwayFromOtherLocations(List<Location> locations, List<Location> players) {
		...
		for (Location location : locations) {
			...
			for (Location player : players) {
				...
				double dist = MathUtils.offsetSquared(player, location);
	...
	public static Location getNearestCornerLocation(Location near, Block block) {
		...
		return AlgebraUtils.findClosest(near, corners);
	}
	...
	public static int randomMidpoint(int min, int max) {
		...
		int value = MathUtils.r(variance);
	...
	public static EulerAngle vectorToEuler(Vector vector) {
		...
		return new EulerAngle(
				Math.toRadians(AlgebraUtils.GetPitch(vector)),
				Math.toRadians(AlgebraUtils.GetYaw(vector)),
				0);
	}
	...
	public static Location getRandomLocation(Location center, double radiusX, double radiusY, double radiusZ) {
		...
		double x = minX + (MathUtils.random.nextDouble() * 2 * radiusX);
		double y = minY + (MathUtils.random.nextDouble() * 2 * radiusY);
		double z = minZ + (MathUtils.random.nextDouble() * 2 * radiusZ);
